...
# Read in results for BS - model A
for (f in files){
...
#meta_results_anc_wide <- res %>% filter(Ancestry == "EUR")
meta_results <- NULL
meta_het <- NULL
...
#fwrite(res_tot, "/Users/germanja/Documents/glp1-weight-loss-jg/results/wq/meta_BS_A_20240424.tsv", sep = "\t")
#fwrite(meta_het, "/Users/germanja/Documents/glp1-weight-loss-jg/results/wq/meta_BS_A_20240424_ancestry_het.tsv", sep = "\t")


# Plot baseline covariate effects across studies by ancestries   ----------


base <- res %>% filter(Exposure == "Baseline") %>% 
  select(-ends_with("_exposure"), -starts_with("P_"))

# Baseline weight
ggplot(base, aes(x = Study ,y = Beta_baseline_body_weight_in_kg)) +
  geom_point() +
  geom_errorbar(aes(ymin = Beta_baseline_body_weight_in_kg - 1.96 * SE_baseline_body_weight_in_kg, ymax = Beta_baseline_body_weight_in_kg + 1.96 * SE_baseline_body_weight_in_kg), width = 0.2) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +
  facet_grid(.~Ancestry) +
  labs(#title = "Effect sizes of baseline weight across studies by ancestries",
    x = "Study",
    y = "Beta (in Kg)") +
  theme_minimal() +
...
# Sex
ggplot(base, aes(x = Study ,y = Beta_sex)) +
  geom_point() +
  geom_errorbar(aes(ymin = Beta_sex - 1.96 * SE_sex, ymax = Beta_sex + 1.96 * SE_sex), width = 0.2) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +
  facet_grid(.~Ancestry) +
  labs(#title = "Effect sizes of sex across studies by ancestries",
    x = "Study",
    y = "Beta (in Kg)") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
# Age
ggplot(base, aes(x = Study ,y = Beta_age_at_initiation_in_years)) +
  geom_point() +
  geom_errorbar(aes(ymin = Beta_age_at_initiation_in_years - 1.96 * SE_age_at_initiation_in_years, ymax = Beta_age_at_initiation_in_years + 1.96 * SE_age_at_initiation_in_years), width = 0.2) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +
  facet_grid(.~Ancestry) +
  labs(#title = "Effect sizes of age at initiation across studies by ancestries",
    x = "Study",
    y = "Beta (in Kg)") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
